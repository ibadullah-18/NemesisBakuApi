...
public class BasketController : ControllerBase
{
    ...
    private readonly IBasketLowStockNotificationQueue
        _lowStockNotificationQueue;

    public BasketController(
        AppDbContext context,
        IBasketLowStockNotificationQueue
            lowStockNotificationQueue)
    {
        _context = context;
        _lowStockNotificationQueue =
            lowStockNotificationQueue;
    ...
    public async Task<IActionResult> GetBasket(CancellationToken cancellationToken)
    {
        ...
        var items = await _context.BasketItems
            ...
            .Select(x => new BasketItemDto
            {
                ...
                Quantity = x.Quantity,
                OriginalTotalPrice = x.Product.Price * x.Quantity,
                TotalPrice =
                    (x.Product.DiscountPrice.HasValue &&
                     x.Product.DiscountPrice.Value > 0 &&
                     x.Product.DiscountPrice.Value < x.Product.Price
        ...
        QueueLowStockNotifications(userId, items);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var productExists = await _context.Products
            .AsNoTracking()
            .AnyAsync(
                x => x.Id == dto.ProductId && x.IsActive,
                cancellationToken);
        ...
        var stockCount = await _context.ProductVariants
            .AsNoTracking()
            .Where(x =>
        ...
        if (!stockCount.HasValue)
        {
            return NotFound(
                ApiResponse<string>.Fail(
                    "Məhsulun seçilmiş razmer/rəngi tapılmadı"));
        }

        if (stockCount.Value <= 0)
        {
            return BadRequest(
                ApiResponse<string>.Fail("Bu məhsul stokda yoxdur"));
        }
        ...
        var updatedRows = await IncrementActiveItemAsync(
            userId,
        ...
        var activeItemExists = await _context.BasketItems
            .AsNoTracking()
            .AnyAsync(
                x =>
                    x.UserId == userId &&
        ...
        var restoredRows = await _context.BasketItems
            .IgnoreQueryFilters()
            .Where(x =>
                x.UserId == userId &&
                x.ProductVariantId == dto.ProductVariantId &&
            ...
            .ExecuteUpdateAsync(
                setters => setters
                    ...
                    .SetProperty(x => x.ProductId, dto.ProductId)
                    .SetProperty(x => x.Quantity, dto.Quantity)
        ...
        {
            // Paralel sorğu sətri əvvəl yaradıbsa, miqdarı atomik artırırıq.
            _context.ChangeTracker.Clear();

            updatedRows = await IncrementActiveItemAsync(
                userId,
                dto.ProductVariantId,
                dto.Quantity,
                stockCount.Value,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var itemInfo = await _context.BasketItems
            .AsNoTracking()
            .Where(x => x.Id == basketItemId && x.UserId == userId)
            .Select(x => new
            {
                VariantIsActive = x.ProductVariant.IsActive,
        ...
        {
            return NotFound(
                ApiResponse<string>.Fail("Səbət məhsulu tapılmadı"));
        }

        if (!itemInfo.VariantIsActive)
        {
            return BadRequest(
                ApiResponse<string>.Fail("Məhsul variantı aktiv deyil"));
        }
        ...
        var updatedRows = await _context.BasketItems
            .Where(x => x.Id == basketItemId && x.UserId == userId)
            .ExecuteUpdateAsync(
                setters => setters
                    .SetProperty(x => x.Quantity, dto.Quantity)
    ...
    {
        return BadRequest(
            ApiResponse<string>.Fail("Stokda kifayət qədər məhsul yoxdur"));
    }
    ...
    private void QueueLowStockNotifications(
        Guid userId,
        IReadOnlyCollection<BasketItemDto> items)
    {
        foreach (var item in items)
        {
            ...
            _lowStockNotificationQueue.TryEnqueue(
                new BasketLowStockNotification(
                    userId,
